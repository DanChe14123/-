...
def play_game(
    ...
    for round_number, task_active, task, event in schedule:
        ...
        if round_number != state["last_upkeep_round"]:
            upkeep_loss = 1 if state["hazards"] >= 3 else 0
    ...
    won = state["progress"] >= mode.progress_target and state["safety"] > 0 and state["hazards"] <= 3
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run balance simulations for boardgame v0.3")
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path(r"C:\PKU\Term2\实验室安全作品大赛\data\v0.3"),
    )
