...
- 当前规则：`C:\PKU\Term2\实验室安全作品大赛\boardgame_v0.2.md`
- 规则归档：`C:\PKU\Term2\实验室安全作品大赛\versions\v0.2\boardgame_rules.md`
- 当前模拟器：`C:\PKU\Term2\实验室安全作品大赛\simulation\run_v0_1_balance.py`
- 模拟器归档：`C:\PKU\Term2\实验室安全作品大赛\versions\v0.2\run_balance_sim.py`
...
- 教学模式通关目标：4 点进度
- 挑战模式通关目标：4 点进度
- 未打出关键行动但打出合理辅助行动时，记为“风险控制成功但任务未推进”
- 低危事件在“基本正确”下不额外扣安全值
...
- 标准模式：所有无序双人组合
- 教学模式：只跑 `balanced+balanced`
- 挑战模式：只跑 `balanced+balanced`
...
```powershell
python "C:\PKU\Term2\实验室安全作品大赛\simulation\run_v0_1_balance.py" `
  --games-per-team 3000 `
  --run-id run_XXX_description `
  --output-root "C:\PKU\Term2\实验室安全作品大赛\data\v0.2"
```
...
- `balanced+balanced` 在教学模式胜率：目标 `60% 以上`
