...
本规程用于让任何后续 AI 或人工测试者，在不重新理解全部上下文的前提下，重复运行《实验室安全值班》`v0.7` 的平衡性测试，并把数据记录到统一格式。
...
```powershell
python "C:\PKU\Term2\实验室安全作品大赛\simulation\run_v0_7_balance.py" `
  --games-per-team 3000 `
  --run-id run_XXX_description `
  --output-root "C:\PKU\Term2\实验室安全作品大赛\data\v0.7"
```
...
当前以“验证事件池扩容成功，并保持标准模式稳定”为目标，优先看以下指标：

- `balanced+balanced` 标准模式胜率：目标 `50%` 到 `56%`
- `balanced+balanced` 教学模式胜率：目标 `50%` 以上，且最好不低于标准模式超过 `3` 个百分点
- `balanced+balanced` 挑战模式胜率：允许明显偏低，但不应接近 `0`
- `reckless+reckless` 标准模式胜率：应明显低于 `balanced+balanced`
- 24 张事件都应稳定出现，不能出现“挂名事件”
- 新增 4 类行动合计选取率：最好保持在 `35%` 左右或更高
- `secure` 选取率：相比 `v0.6` 应有明显提升
...
1. 先判断是事件难度问题，还是任务映射问题。
2. 若标准模式胜率过低，优先检查：
   - 新事件是否过多要求“任务不偏好的关键行动”
   - 低危事件是否过于拖慢进度
   - 新事件是否过度增加了 `contained` 结果
3. 若 `secure / inspect / evacuate` 选取率仍偏低，优先检查：
   - 是否有足够多事件把它们设为关键行动
   - 对应任务是否也把它们列为偏好行动
4. 若教学模式优势消失，优先回调：
   - 教学模式初始安全值
...
- 不要覆盖历史 `run_id`
- 不要直接改 `versions/v0.1` 到 `versions/v0.7` 中的归档文件
- 不要在未留档的情况下直接覆盖当前规则
- 不要只写结论而不保留原始数据
