...
SAFE_ACTIONS = {"stop", "report", "emergency", "shutdown", "evacuate"}
HAND_SIZE = 8
...
TASKS: Tuple[Task, ...] = (
    Task("T01", "配制酸溶液", ("ppe", "label", "emergency"), "recover_safety", ("E01", "E02", "E13", "E17")),
    Task("T02", "有机溶剂转移", ("hood", "shutdown", "label", "evacuate"), "clear_hazard", ("E05", "E06", "E16", "E22")),
    Task("T03", "加热回流", ("shutdown", "stop", "report", "emergency"), "reduce_upgrade", ("E06", "E12", "E14", "E15", "E24")),
    Task("T04", "废液分类处理", ("waste", "label", "cleanup", "evacuate"), "clear_hazard", ("E04", "E11", "E16", "E23")),
    Task("T05", "气瓶连接使用", ("secure", "inspect", "report"), "reduce_upgrade", ("E08", "E09", "E19", "E24")),
    Task("T06", "通风橱内称量与转移", ("hood", "cleanup", "label"), "gain_redraw", ("E03", "E05", "E10", "E16", "E22")),
    Task("T07", "清理实验台", ("cleanup", "waste", "inspect", "evacuate"), "recover_safety", ("E03", "E07", "E08", "E23")),
    Task("T08", "紧急情况报告与撤离", ("evacuate", "report", "emergency"), "control_choice", ("E11", "E13", "E15", "E16", "E23", "E24")),
    Task("T09", "危险化学品领用与登记", ("label", "report", "inspect"), "reduce_upgrade", ("E02", "E08", "E17")),
    Task("T10", "低温/高温设备使用", ("ppe", "shutdown", "secure", "emergency"), "recover_safety", ("E01", "E12", "E13", "E18", "E19")),
    Task("T11", "旋蒸或离心设备操作", ("inspect", "secure", "shutdown", "hood"), "gain_redraw", ("E05", "E08", "E20", "E21", "E22")),
    Task("T12", "实验结束后的交接检查", ("cleanup", "waste", "shutdown", "inspect", "label"), "clear_hazard", ("E03", "E04", "E10", "E17", "E18")),
)
...
EVENTS: Tuple[Event, ...] = (
    ...
    Event("E06", "有机溶剂附近存在火源", "medium", "shutdown", ("stop", "hood")),
    Event("E07", "地面少量液体洒漏", "medium", "cleanup", ("report",), (("stop", "report"),)),
    Event("E08", "玻璃器皿出现裂纹", "medium", "inspect", ("stop",)),
    Event("E09", "气瓶未固定", "medium", "secure", ("stop",), (("stop", "report"),)),
    Event("E10", "通风橱内堆放过多物品", "medium", "cleanup", ("hood",), (("stop", "report"),)),
    Event("E11", "废液混放产生异常气味", "high", "evacuate", ("report",), (("stop", "report"),)),
    Event("E12", "加热装置无人值守", "high", "shutdown", ("stop", "report")),
    Event("E13", "皮肤接触腐蚀性液体", "high", "emergency", ("report",), urgent_only=True),
    Event("E14", "容器内压力异常升高", "high", "shutdown", ("evacuate", "report"), (("evacuate", "report"),)),
    Event("E15", "实验台出现小型明火", "high", "emergency", ("shutdown", "report"), urgent_only=True),
    Event("E16", "刺激性气味明显增强", "high", "evacuate", ("hood", "report"), (("stop", "report"),)),
    Event("E17", "危化品领用记录与实物不一致", "low", "label", ("report", "inspect")),
    Event("E18", "实验结束后设备未断电且记录缺失", "low", "shutdown", ("label", "inspect")),
    Event("E19", "装置软管连接松动并有渗漏", "medium", "secure", ("shutdown",), (("stop", "report"),)),
    Event("E20", "离心样品配平异常", "medium", "inspect", ("shutdown", "stop")),
    Event("E21", "旋蒸装置夹具松动或收集瓶偏斜", "medium", "secure", ("inspect", "stop")),
    Event("E22", "通风橱前窗开启过高导致气流失效", "medium", "hood", ("cleanup", "stop")),
    Event("E23", "未知粉末散落并引发刺激性不适", "high", "evacuate", ("report",), (("stop", "report"),)),
    Event("E24", "钢瓶减压阀异常结霜并伴异响", "high", "evacuate", ("shutdown", "report"), (("shutdown", "report"),)),
)


MODES: Dict[str, Mode] = {
    "teaching": Mode("teaching", initial_safety=10, initial_hazards=0, redraws=4, progress_target=3, extra_event_rounds=(), first_error_no_upgrade=True),
...
def score_action(
    ...
    if hazards >= 2 and action in SAFE_ACTIONS:
        score += 12
    if action == "report":
        if event.severity == "high" or event.urgent_only or hazards >= 2:
            score += profile.conservative_bias
        else:
            score += max(0, profile.conservative_bias - 5)
    if action == "stop":
        if event.severity == "medium":
            score += profile.conservative_bias
        elif event.severity == "high":
            score += max(0, profile.conservative_bias - 1)
        else:
            score += max(0, profile.conservative_bias - 4)
    if action == "shutdown":
        if event.severity in {"medium", "high"}:
            score += profile.conservative_bias + 6
    if action == "evacuate":
        if event.severity == "high":
            score += profile.conservative_bias + 8
    if action == "cleanup" and hazards >= 2:
        score += 6
    if already_chosen:
...
def should_redraw(profile: BotProfile, task: Task | None, event: Event, hand: Sequence[str], redraws_left: int, hazards: int) -> bool:
    ...
    for combo in event.containment_combos:
        if set(combo).issubset(hand_set):
            return False
    if task and any(action in hand_set for action in task.preferred_actions) and event.severity == "low" and hazards <= 1 and event.key_action not in {"shutdown", "secure"}:
        return False
    if task and sum(1 for action in hand_set if action in task.preferred_actions) >= 2 and event.severity != "high":
        return False
    return True
...
def play_game(
    ...
    for round_number, task_active, task, event in schedule:
        ...
        if evaluation["outcome"] == "full":
            ...
            if not (mode.name == "teaching" and event.severity == "low" and evaluation["outcome"] == "contained"):
                state["hazards"] += 1
        ...
        if strategy_id == "risk_isolation" and evaluation["outcome"] in {"full", "partial"} and any(action in {"stop", "cleanup", "evacuate"} for action in chosen_actions):
            state["redraws_left"] += 1
            state["redraws_gained_total"] += 1
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run balance simulations for boardgame v0.8")
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path(r"C:\PKU\Term2\实验室安全作品大赛\data\v0.8"),
    )
