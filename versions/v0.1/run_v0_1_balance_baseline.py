...
EVENTS: Tuple[Event, ...] = (
    ...
    Event("E14", "容器内压力异常升高", "high", "stop", ("report",), major_accident_if_missing_key=True),
...
MODES: Dict[str, Mode] = {
    "teaching": Mode("teaching", initial_safety=9, redraws=3, progress_target=5, extra_event_rounds=(), first_error_no_upgrade=True),
    "standard": Mode("standard", initial_safety=8, redraws=2, progress_target=5, extra_event_rounds=()),
    "challenge": Mode("challenge", initial_safety=7, redraws=1, progress_target=5, extra_event_rounds=(3, 5)),
}
...
BOT_PROFILES: Dict[str, BotProfile] = {
    ...
    "balanced": BotProfile("balanced", 92, 62, 35, 42, 6, 24, False, True, False, noise_rate=0.04, conservative_bias=5),
...
def play_game(
    ...
    for round_number, task_active, task, event in schedule:
        ...
        hand.extend(deck.draw(5 - len(hand)))
    ...
    for mode, team in run_matrix:
        ...
        for game_index in range(games_per_team):
            ...
            for turn_log in result["turn_logs"]:
                event_counter[turn_log["event_id"]] += 1
                event_outcomes[turn_log["event_id"]][turn_log["outcome"]] += 1
                for action in turn_log["chosen_actions"]:
